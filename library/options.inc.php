    ...
    while ($lrow = sqlFetchArray($lres)) {
        ...
        if ($outtype == 3) {
            if (isset($avalue[$option_id][1]) && $avalue[$option_id][1] == '2') {
                ...
                $s .= ':' . $avalue[$option_id][0];
                $s .= ':' . $avalue[$option_id][2];
                $s .= ':' . $avalue[$option_id][3];
            }
            continue;
        }
...
function getCodeDescription($codestring, $defaulttype='ICD10')
{
    ...
    $query = "SELECT c.code_text FROM codes AS c, code_types AS ct WHERE " .
        "ct.ct_key = ? AND c.code_type = ct.ct_id AND c.code = ? AND c.active = 1 " .
        "ORDER BY c.id LIMIT 1";
    $nrow = sqlQuery($query, array($ctype, $code));
...
function display_layout_tabs_data_editable($formtype, $result1, $result2 = '')
{
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            /**********************************************************
            if (!isset($grparr[$this_group])) {
                // This should not happen.
                $grparr[$this_group] = array(
                    'grp_group_id' => $this_group,
                    'grp_init_open' => '0',
                    'grp_title' => '***' . xl('Undefined') . '***',
                    'grp_subtitle' => '',
                );
            }
            **********************************************************/
                ...
                if ($gs_i <= 1) {
                    ...
                    if (strlen($gs_group_name)) {
                        echo "<br /><span class='bold'><input type='checkbox' name='form_cb_" .
                            ...
                            "onclick='return divclick(this,\"div_" .
                            attr(addslashes($gs_group_seq)) . "\");'";
